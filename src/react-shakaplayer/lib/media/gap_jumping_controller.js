...
class GapJumpingController {
	...
	onPollGapJump_() {
		...
		const gapIndex = TimeRangesUtils.getGapIndex(buffered, currentTime);
		...
		if (!isGapSmall && !this.didFireLargeGap_) {
			...
			const event = new FakeEvent('largegap', { currentTime: currentTime, gapSize: jumpSize });
		...
		if (isGapSmall || jumpLargeGap) {
			if (gapIndex == 0) {
				shaka.log.info('Jumping forward', jumpSize, 'seconds because of gap before start time of', jumpTo);
			} else {
				shaka.log.info(
					'Jumping forward',
					jumpSize,
...
export default GapJumpingController;
