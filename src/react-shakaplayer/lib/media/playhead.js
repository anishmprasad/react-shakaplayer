...
class SrcEqualsPlayhead {
	...
	constructor(mediaElement) {
		...
		if (this.mediaElement_.readyState == 0) {
			this.eventManager_.listenOnce(this.mediaElement_, 'loadedmetadata', onLoaded);
		} else {
			// It's already loaded.
			onLoaded();
		}
...
class MediaSourcePlayhead {
	...
	onPollWindow_() {
		...
		if (currentTime < seekStart) {
			...
			shaka.log.info(
				'Jumping forward ' + (targetTime - currentTime) + ' seconds to catch up with the seek range.'
			);
	...
	createStallDetector_(mediaElement, config) {
		...
		detector.onStall((at, duration) => {
			shaka.log.debug(
				['Stall detected at', at, 'for', duration, 'seconds. Seeking forward', skip, 'seconds.'].join(' ')
			);
