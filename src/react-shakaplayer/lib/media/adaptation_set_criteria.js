...
class PreferenceBasedCriteria {
	...
	create(variants) {
		...
		if (this.channelCount_) {
			...
			if (byChannel.length) {
				...
				shaka.log.warning('No exact match for the channel count could be found.');
			}
		}
	...
	static filterVariantsByRole_(variants, preferredRole) {
		return variants.filter(variant => {
			...
			return (
				(audio && audio.roles.indexOf(preferredRole) >= 0) || (video && video.roles.indexOf(preferredRole) >= 0)
			);
		});
	}
}
