...
shaka.media.ManifestParser = class {
	...
	static async getFactory_(uri, netEngine, retryParams, mimeType) {
		...
		if (mimeType) {
			...
			shaka.log.warning('Could not determine manifest type using MIME type ', mimeType);
		}
		...
		if (extension) {
			...
			shaka.log.warning('Could not determine manifest type for extension ', extension);
		} else {
