...
class VideoWrapper {
	 ...
	 */
	getTime() {
		return this.video_.readyState > 0 ? this.video_.currentTime : this.startTime_;
	}
...
class PlayheadMover {
	...
	onTick_() {
		...
		if (this.remainingAttempts_ <= 0) {
			shaka.log.warning(['Failed to move playhead from', this.originTime_, 'to', this.targetTime_].join(' '));
