...
import { PlaylistType, Playlist, Segment, Tag, Attribute } from '../hls/hls_classes';
...
export default class ManifestTextParser {
	...
	parsePlaylist(data, absolutePlaylistUri) {
		const MEDIA_PLAYLIST_TAGS = ManifestTextParser.MEDIA_PLAYLIST_TAGS;
		...
		if (!/^#EXTM3U($|[ \t\n])/m.test(lines[0])) {
			throw new Error(Error.Severity.CRITICAL, Error.Category.MANIFEST, Error.Code.HLS_PLAYLIST_HEADER_MISSING);
		}
		...
		for (let i = 1; i < lines.length; ) {
			...
			if (tag.name == 'EXT-X-STREAM-INF') {
				const tagUri = new Attribute('URI', lines[i]);
	...
	parseSegments_(absoluteMediaPlaylistUri, lines, playlistTags) {
		...
		for (const line of lines) {
			if (/^(#EXT)/.test(line)) {
				...
				if (ManifestTextParser.MEDIA_PLAYLIST_TAGS.includes(tag.name)) {
				...
				const segment = new Segment(absoluteSegmentUri, segmentTags);
    ...
    */
		...
		if (!blocks) {
			throw new Error(Error.Severity.CRITICAL, Error.Category.MANIFEST, Error.Code.INVALID_HLS_TAG, word);
		}
		const name = blocks[1];
		const data = blocks[2];
		const attributes = [];
