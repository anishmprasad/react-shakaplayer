...
class ManifestFilter {
	...
	static filterByMediaSourceSupport(manifest) {
		...
		for (const period of manifest.periods) {
			period.variants = period.variants.filter(variant => {
				let supported = true;
				if (variant.audio) {
					supported = supported && MediaSourceEngine.isStreamSupported(variant.audio);
				}
				if (variant.video) {
					supported = supported && MediaSourceEngine.isStreamSupported(variant.video);
				}
				return supported;
			});
		}
	...
	static filterByCommonCodecs(manifest) {
		...
		for (const period of manifest.periods) {
			/** @type {!shaka.util.ManifestFilter.VariantCodecSummarySet} */
			const next = ManifestFilter.VariantCodecSummarySet.fromVariants(period.variants);
...
ManifestFilter.VariantCodecSummary = class {
	...
	equals(other) {
		return (
			this.audioMime_ == other.audioMime_ &&
