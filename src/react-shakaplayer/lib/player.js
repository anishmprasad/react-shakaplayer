...
import MediaSourceEngine from '../lib/media/media_source_engine';
...
import Walker from '../lib/routing/walker';
import SimpleTextDisplayer from '../lib/text/simple_text_displayer';

import ArrayUtils from '../lib/util/array_utils';
...
import ManifestParserUtils from '../lib/util/manifest_parser_utils';
import MimeUtils from '../lib/util/mime_utils';
import MultiMap from '../lib/util/multi_map';
import Periods from '../lib/util/periods';
...
export default class Player extends FakeEventTarget {
	...
	constructor(mediaElement, dependencyInjector) {
		...
		const walkerImplementation = {
			...
			enterNode: (node, has, wants) => {
				this.dispatchEvent(new FakeEvent(/* name= */ 'onstatechange', /* data= */ { state: node.name }));
			...
			onIdle: node => {
				this.dispatchEvent(new FakeEvent(/* name= */ 'onstateidle', /* data= */ { state: node.name }));
			}
		};
	...
	load(assetUri, startTime, mimeType) {
		...
		if (mimeType && typeof mimeType != 'string') {
			...
			const Factory = /** @type {shaka.extern.ManifestParser.Factory} */ (mimeType);
	...
	async onInitializeParser_(has, wants) {
		...
		if (has.factory) {
			...
			this.parser_ = await ManifestParser.create(
				assetUri,
				networkingEngine,
	...
	onParseManifest_(has, wants) {
		...
		this.regionTimeline_.setListeners(
	...
	async onInitializeDrm_(has, wants) {
		...
		await this.drmEngine_.initForPlayback(
			Periods.getAllVariantsFrom(this.manifest_.periods),
	...
	async onLoad_(has, wants) {
		...
		this.playRateController_ = new PlayRateController({
			...
			setRate: rate => {
				has.mediaElement.playbackRate = rate;
			},
			movePlayhead: delta => {
				has.mediaElement.currentTime += delta;
			}
		});
		...
		if (!this.config_.preferredAudioLanguage && !hasPrimary) {
			shaka.log.warning('No preferred audio language set.  We will choose an ' + 'arbitrary language initially');
		}
	...
	static filterForAVVariants_(periods) {
		...
		if (hasAVVariant) {
			shaka.log.debug(
				'Found variant with audio and video content, ' + 'so filtering out audio-only content in all periods.'
			);
	...
	chooseCodecsAndFilterManifest_() {
		...
		let variants = this.manifest_.periods.reduce((variants, period) => variants.concat(period.variants), []);
		...
		variants = StreamUtils.filterVariantsByAudioChannelCount(variants, this.config_.preferredAudioChannelCount);
		...
		/** @type {!MultiMap.<shaka.extern.Variant>} */
	...
	createDrmEngine(playerInterface) {
		return new DrmEngine(playerInterface);
	}
	...
	applyConfig_() {
		...
		if (this.streamingEngine_) {
			...
			const activeVariant = StreamUtils.getVariantByStreams(activeAudio, activeVideo, period.variants);
			if (
				this.abrManager_ &&
				activeVariant &&
	...
	resetConfiguration() {
		...
		PlayerConfiguration.mergeConfigObjects(this.config_, this.defaultConfig_(), this.defaultConfig_());
		this.applyConfig_();
	}
	...
	isBuffering() {
		...
		return this.bufferObserver_ ? this.bufferObserver_.getState() == State.STARVING : false;
	}
	...
	getVariantTracks() {
		if (this.manifest_ && this.playhead_) {
			...
			return audioTracks.map(audio => StreamUtils.html5AudioTrackToTrack(audio));
		} else {
			return [];
		}
	}
	...
	selectTextTrack(track) {
		if (this.manifest_ && this.streamingEngine_) {
			...
			this.addTextStreamToSwitchHistory_(period, stream, /* fromAdaptation= */ false);
	...
	selectEmbeddedTextTrack() {
		...
		const tracks = this.getTextTracks().filter(track => {
			return track.mimeType == MimeUtils.CLOSED_CAPTION_MIMETYPE;
		});
	...
	usingEmbeddedTextTrack() {
		...
		if (activeTrack) {
			return activeTrack.mimeType == MimeUtils.CLOSED_CAPTION_MIMETYPE;
		}
	...
	selectAudioLanguage(language, role) {
		if (this.manifest_ && this.playhead_) {
			...
			this.currentAdaptationSetCriteria_ = new PreferenceBasedCriteria(language, role || '', 0);
	...
	getPlayheadTimeAsDate() {
		...
		if (this.manifest_) {
			...
			if (isNaN(startDate.getTime())) {
				shaka.log.warning('EXT-X-PROGRAM-DATETIME required to get playhead time as Date!');
				return null;
			}
	...
	getPresentationStartTimeAsDate() {
		...
		if (this.manifest_) {
			...
			const startDate = this.video_.getStartDate();
			if (isNaN(startDate.getTime())) {
				shaka.log.warning('EXT-X-PROGRAM-DATETIME required to get presentation start time ' + 'as Date!');
				return null;
			}
			return startDate;
		} else {
	...
	async addTextTrack(uri, language, kind, mime, codec, label) {
		...
		const nextPeriodStart =
			nextPeriodIndex >= this.manifest_.periods.length
		...
		const segmentReference = new SegmentReference(1, 0, periodDuration, () => [uri], 0, null);
	...
	filterAllPeriods_(periods) {
		...
		for (const period of periods) {
			StreamUtils.filterNewPeriod(this.drmEngine_, activeAudio, activeVideo, period);
		}
		...
		for (const period of periods) {
			...
			if (tracksChanged && this.streamingEngine_ && this.getPresentationPeriod_() == period) {
				this.onTracksChanged_();
			}
	...
	filterNewPeriod_(period) {
		...
		const tracksChanged = StreamUtils.applyRestrictions(variants, this.config_.restrictions, this.maxHwRes_);
	...
	assertCorrectActiveStreams_() {
		...
		const expectedAudio = this.deferredVariant_ ? this.deferredVariant_.audio : activeAudio;

		const expectedVideo = this.deferredVariant_ ? this.deferredVariant_.video : activeVideo;
	...
	updateBufferState_() {
		...
		// buffering.
		const event = new FakeEvent('buffering', { buffering: isBuffering });
	...
	chooseStreamsAndSwitch_(period) {
		...
		if (chosenVariant) {
			this.addVariantToSwitchHistory_(period, chosenVariant, /* fromAdaptation= */ true);
		...
		if (chosenText && this.shouldStreamText_()) {
			this.addTextStreamToSwitchHistory_(period, chosenText, /* fromAdaptation= */ true);
	...
	canSwitch_() {
		...
		if (this.deferredVariant_) {
			this.streamingEngine_.switchVariant(
				this.deferredVariant_,
				this.deferredVariantClearBuffer_,
	...
	onAdaptation_() {
		...
		this.delayDispatchEvent_(new FakeEvent('adaptation'));
	}
	...
	onTracksChanged_() {
		...
		this.delayDispatchEvent_(new FakeEvent('trackschanged'));
	}
	...
	onVariantChanged_() {
		...
		// the changes before the user tries to query it.
		this.delayDispatchEvent_(new FakeEvent('variantchanged'));
	}
	...
	onTextChanged_() {
		...
		// changes before the user tries to query it.
		this.delayDispatchEvent_(new FakeEvent('textchanged'));
	}

	/** @private */
	onTextTrackVisibility_() {
		this.delayDispatchEvent_(new FakeEvent('texttrackvisibility'));
	}

	/** @private */
	onAbrStatusChanged_() {
		this.delayDispatchEvent_(
			new FakeEvent('abrstatuschanged', {
	...
	onRegionEvent_(eventName, region) {
		...
		this.dispatchEvent(new FakeEvent(eventName, { detail: clone }));
	}
			...
			for (const variant of period.variants) {
				...
				for (const stream of streams) {
					...
					if (stream.keyId) {
						...
						variant.allowedByKeySystem = !!keyStatus && !restrictedStatuses.includes(keyStatus);
					}
	...
	onExpirationUpdated_(keyId, expiration) {
		...
		const event = new FakeEvent('expirationupdated');
		this.dispatchEvent(event);
	}
	...
	static applyPlayRange_(timeline, playRangeStart, playRangeEnd) {
		if (playRangeStart > 0) {
			if (timeline.isLive()) {
				shaka.log.warning('|playRangeStart| has been configured for live content. ' + 'Ignoring the setting.');
			} else {
		...
		if (playRangeEnd < fullDuration) {
			if (timeline.isLive()) {
				shaka.log.warning('|playRangeEnd| has been configured for live content. ' + 'Ignoring the setting.');
			} else {
	...
	getNextStep_(currentlyAt, currentlyWith, wantsToBeAt, wantsToHave) {
		...
		if (currentlyAt == this.detachNode_) {
			next = wantsToBeAt == this.detachNode_ ? this.detachNode_ : this.attachNode_;
		}
		...
		if (currentlyAt == this.mediaSourceNode_) {
			next = this.getNextAfterMediaSource_(wantsToBeAt, currentlyWith, wantsToHave);
		}
		...
		if (currentlyAt == this.srcEqualsDrmNode_) {
			if (wantsToBeAt == this.srcEqualsNode_ && currentlyWith.mediaElement == wantsToHave.mediaElement) {
	...
	getNextAfterUnload_(goingTo, has, wants) {
		...
		// do so.
		return !wants.mediaElement || has.mediaElement != wants.mediaElement ? this.detachNode_ : this.attachNode_;
	}
	 ...
	 * @private
	 */
	getNextMatchingAllDependencies_(destinationNode, nextNode, resetNode, goingTo, has, wants) {
		if (
