...
export default class CastProxy extends FakeEventTarget {
	 ...
	 */
	canCast() {
		return this.sender_ ? this.sender_.apiReady() && this.sender_.hasReceivers() : false;
	}
	...
	init_() {
		...
		for (const name of CastUtils.VideoEvents) {
			this.eventManager_.listen(this.localVideo_, name, event => this.videoProxyLocalEvent_(event));
		}

		for (const name of CastUtils.PlayerEvents) {
			this.eventManager_.listen(this.localPlayer_, name, event => this.playerProxyLocalEvent_(event));
		}
		...
		this.videoEventTarget_.dispatchTarget = /** @type {EventTarget} */ (this.videoProxy_);

		this.playerEventTarget_ = new FakeEventTarget();
		this.playerEventTarget_.dispatchTarget = /** @type {EventTarget} */ (this.playerProxy_);
	}
	...
	playerProxyGet_(name) {
		...
		if (name == 'getSharedConfiguration') {
			shaka.log.warning("Can't share configuration across a network. Returning copy.");
			return this.sender_ ? this.sender_.get('player', 'getConfiguration') : this.localPlayer_.getConfiguration();
		}
