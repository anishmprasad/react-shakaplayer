...
class TextEngine {
	 ...
	 */
	storeAndAppendClosedCaptions(closedCaptions, startTime, endTime, videoTimestampOffset) {
		...
		for (const caption of closedCaptions) {
			...
			const keepThisCue =
				caption.startTime >= this.appendWindowStart_ && caption.startTime < this.appendWindowEnd_;
			if (!keepThisCue) {
				continue;
			}
			/** @type {!shaka.text.Cue} */
