...
class TtmlTextParser {
	...
	parseMedia(data, time) {
		...
		if (xml) {
			...
			if (!tt) {
				...
				frameRateMultiplier = XmlUtils.getAttributeNS(tt, ttpNs, 'frameRateMultiplier');
			...
			const metadataElements = TtmlTextParser.getLeafNodes_(tt.getElementsByTagName('metadata')[0]);
			const styles = TtmlTextParser.getLeafNodes_(tt.getElementsByTagName('styling')[0]);
			const regionElements = TtmlTextParser.getLeafNodes_(tt.getElementsByTagName('layout')[0]);
			const cueRegions = [];
			for (let i = 0; i < regionElements.length; i++) {
				const cueRegion = TtmlTextParser.parseCueRegion_(regionElements[i], styles, extent);
				if (cueRegion) {
					cueRegions.push(cueRegion);
				}
			}

			const textNodes = TtmlTextParser.getLeafNodes_(tt.getElementsByTagName('body')[0]);
	...
	static parseCue_(
		...
		// Get time.
		let start = shaka.text.TtmlTextParser.parseTime_(cueElement.getAttribute('begin'), rateInfo);
		let end = shaka.text.TtmlTextParser.parseTime_(cueElement.getAttribute('end'), rateInfo);
		const duration = shaka.text.TtmlTextParser.parseTime_(cueElement.getAttribute('dur'), rateInfo);
		...
		shaka.text.TtmlTextParser.addStyle_(cue, cueElement, regionElement, imageElement, styles);
	...
	static parseCueRegion_(regionElement, styles, globalExtent) {
		...
		if (!id) {
			shaka.log.warning('TtmlTextParser parser encountered a region with ' + 'no id. Region will be ignored.');
			return null;
		}
		region.id = id;
		...
		if (globalExtent) {
			globalResults =
				TtmlTextParser.percentValues_.exec(globalExtent) || TtmlTextParser.pixelValues_.exec(globalExtent);
		}
	...
	static addStyle_(cue, cueElement, region, imageElement, styles) {
		...
		const direction = TtmlTextParser.getStyleAttribute_(cueElement, region, styles, 'direction');
		if (direction == 'rtl') {
		...
		const writingMode = TtmlTextParser.getStyleAttribute_(cueElement, region, styles, 'writingMode');
		...
		const align = TtmlTextParser.getStyleAttribute_(cueElement, region, styles, 'textAlign');
		if (align) {
		...
		const color = TtmlTextParser.getStyleAttribute_(cueElement, region, styles, 'color');
		if (color) {
			cue.color = color;
		}

		const backgroundColor = TtmlTextParser.getStyleAttribute_(cueElement, region, styles, 'backgroundColor');
		if (backgroundColor) {
			cue.backgroundColor = backgroundColor;
		}

		const fontFamily = TtmlTextParser.getStyleAttribute_(cueElement, region, styles, 'fontFamily');
		if (fontFamily) {
			cue.fontFamily = fontFamily;
		}

		const fontWeight = TtmlTextParser.getStyleAttribute_(cueElement, region, styles, 'fontWeight');
		if (fontWeight && fontWeight == 'bold') {
			cue.fontWeight = Cue.fontWeight.BOLD;
		}

		const wrapOption = TtmlTextParser.getStyleAttribute_(cueElement, region, styles, 'wrapOption');
		if (wrapOption && wrapOption == 'noWrap') {
			cue.wrapLine = false;
		}

		const lineHeight = TtmlTextParser.getStyleAttribute_(cueElement, region, styles, 'lineHeight');
		if (lineHeight && lineHeight.match(TtmlTextParser.unitValues_)) {
			cue.lineHeight = lineHeight;
		}

		const fontSize = TtmlTextParser.getStyleAttribute_(cueElement, region, styles, 'fontSize');
	...
	static getStyleAttributeFromRegion_(region, styles, attribute) {
		...
		const style = shaka.text.TtmlTextParser.getElementFromCollection_(region, 'style', styles, /* prefix= */ '');
		if (style) {
	...
	static getStyleAttributeFromElement_(cueElement, styles, attribute) {
		...
		const getElementFromCollection_ = shaka.text.TtmlTextParser.getElementFromCollection_;
		const style = getElementFromCollection_(cueElement, 'style', styles, /* prefix= */ '');
		if (style) {
			return XmlUtils.getAttributeNS(style, ttsNs, attribute);
		}
		return null;
	 ...
	 * @private
	 */
	static getElementFromCollection_(element, attributeName, collection, prefixName) {
		if (!element || collection.length < 1) {
			return null;
		}
		let item = null;
		const itemName = shaka.text.TtmlTextParser.getInheritedAttribute_(element, attributeName);
		if (itemName) {
 ...
 * @example 50% 10%
 */
TtmlTextParser.percentValues_ = /^(\d{1,2}|100)% (\d{1,2}|100)%$/;
 ...
 * @example 100px
 */
TtmlTextParser.unitValues_ = /^(\d+px|\d+em)$/;
 ...
 */
TtmlTextParser.timeColonFormatFrames_ = /^(\d{2,}):(\d{2}):(\d{2}):(\d{2})\.?(\d+)?$/;
 ...
 */
TtmlTextParser.timeColonFormatMilliseconds_ = /^(?:(\d{2,}):)?(\d{2}):(\d{2}\.\d{2,})$/;
 ...
 */
TtmlTextParser.timeHMSFormat_ = new RegExp(
	[
		'^(?:(\\d*(?:\\.\\d*)?)h)?',
		'(?:(\\d*(?:\\.\\d*)?)m)?',
		'(?:(\\d*(?:\\.\\d*)?)s)?',
...
TextEngine.registerParser('application/ttml+xml', TtmlTextParser);

export default TtmlTextParser;
