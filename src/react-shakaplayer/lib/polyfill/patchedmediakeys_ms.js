...
export default class PatchedMediaKeysMs {
	...
	static install() {
		...
		navigator.requestMediaKeySystemAccess = PatchedMediaKeysMs.requestMediaKeySystemAccess;
		// eslint-disable-next-line no-restricted-syntax
		HTMLMediaElement.prototype.setMediaKeys = PatchedMediaKeysMs.MediaKeySystemAccess.setMediaKeys;
	}
	...
	static onMsNeedKey_(event) {
		...
		const event2 = /** @type {!CustomEvent} */ (document.createEvent('CustomEvent'));
	...
	static normaliseInitData_(initData) {
		...
		for (const dataBoundary of pssh.dataBoundaries) {
			const currPssh = initData.subarray(dataBoundary.start, dataBoundary.end + 1); // End is exclusive, hence the +1.
...
shaka.polyfill.PatchedMediaKeysMs.MediaKeySystemAccess = class {
	...
	static setMediaKeys(mediaKeys) {
		...
		const newMediaKeys = /** @type {shaka.polyfill.PatchedMediaKeysMs.MediaKeys} */ (mediaKeys);
