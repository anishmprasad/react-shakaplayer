...
shaka.polyfill.PatchedMediaKeysApple = class {
    ...
    */
		...
		PatchedMediaKeysApple.MediaKeyStatusMap.KEY_ID_ = new Uint8Array([0]).buffer;
	...
	static setMediaKeys(mediaKeys) {
		...
		const newMediaKeys = /** @type {shaka.polyfill.PatchedMediaKeysApple.MediaKeys} */ (mediaKeys);
	...
	static rebuildInitData_(initData, certificate) {
		...
		const length = dataview.getUint32(/* position= */ 0, /* littleEndian= */ true);
	...
	static onWebkitNeedKey_(event) {
		...
		event2.initData = PatchedMediaKeysApple.rebuildInitData_(event.initData, certificate);
...
shaka.polyfill.PatchedMediaKeysApple.MediaKeys = class {
	...
	setServerCertificate(serverCertificate) {
		...
		this.certificate = serverCertificate ? new Uint8Array(serverCertificate) : null;
...
shaka.polyfill.PatchedMediaKeysApple.MediaKeySession = class extends shaka.util.FakeEventTarget {
	...
	generateRequest(initDataType, initData) {
		...
		try {
			...
			const session = this.nativeMediaKeys_.createSession('video/mp4', new Uint8Array(initData));
	...
	remove() {
		...
		return Promise.reject(
			new Error(
				'MediaKeySession.remove is only ' +
