...
shaka.polyfill.PiPWebkit = class {
	...
	static install() {
		...
		document.addEventListener('webkitpresentationmodechanged', PiPWebkit.proxyEvent_, /* useCapture= */ true);
	}
	...
	static exitPictureInPicture_() {
		...
		const pipElement = /** @type {HTMLVideoElement} */ (document.pictureInPictureElement);
		if (pipElement) {
			// Exit PiP mode.
