...
shaka.dash.SegmentBase = class {
	...
	static createStream(context, requestInitSegment) {
		...
		const init = SegmentBase.createInitSegment(context, SegmentBase.fromInheritance_);
		const index = SegmentBase.createSegmentIndex_(context, requestInitSegment, init, scaledPresentationTimeOffset);
	...
	static createSegmentIndexFromUris(
		...
		const create = async () => {
			const responses = [
				...
				containerType == 'webm' ? localRequest(init.getUris(), init.startByte, init.endByte) : null
			];
