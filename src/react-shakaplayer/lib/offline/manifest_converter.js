...
class ManifestConverter {
	...
	fromPeriodDB(period, timeline) {
		...
		const audioStreams = period.streams.filter(stream => this.isAudio_(stream));
		/** @type {!Array.<shaka.extern.StreamDB>} */
		const videoStreams = period.streams.filter(stream => this.isVideo_(stream));
	...
	fromStreamDB_(streamDB) {
		...
		if (streamDB.initSegmentKey != null) {
			stream.initSegmentReference = this.fromInitSegmentDB_(streamDB.initSegmentKey);
		}
