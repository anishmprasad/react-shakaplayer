...
const ShakaDemoAssetInfo = class {
	...
	constructor(name, iconUri, manifestUri, source) {
		...
		/** @type {?function()} */
		this.storeCallback = noop;
		/** @type {?function()} */
		this.unstoreCallback = noop;
		/** @type {?shaka.extern.StoredContent} */
		this.storedContent = noop;
		/** @type {number} */
	...
	applyFilters(networkingEngine) {
		...
		if (this.licenseRequestHeaders.size) {
			const filter = (requestType, request) => {
				return this.addLicenseRequestHeaders_(this.licenseRequestHeaders, requestType, request);
			};
	 ...
	 */
	getConfiguration() {
		const config = /** @type {shaka.extern.PlayerConfiguration} */ ({ drm: {}, manifest: { dash: {} } });
