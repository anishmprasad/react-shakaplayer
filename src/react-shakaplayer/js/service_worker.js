...
console.assert(CACHE_NAME.startsWith(CACHE_NAME_PREFIX), 'Cache name does not match prefix!');
...
function onActivate(event) {
	...
	const dropOldCaches = async () => {
		...
		const cleanTheseUp = cacheNames.filter(
			cacheName => cacheName.startsWith(CACHE_NAME_PREFIX) && cacheName != CACHE_NAME
		);

		const cleanUpPromises = cleanTheseUp.map(cacheName => caches.delete(cacheName));
...
function onFetch(event) {
	...
	if (!useCache) {
		if (CRITICAL_RESOURCES.includes(event.request.url) || OPTIONAL_RESOURCES.includes(event.request.url)) {
			useCache = true;
		}
	}
