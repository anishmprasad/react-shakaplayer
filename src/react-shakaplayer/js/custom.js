...
export default class ShakaDemoCustom {
	...
	constructor(container) {
		...
		document.addEventListener('shaka-main-page-changed', () => {
			if (!this.savedList_.childNodes.length && !container.classList.contains('hidden')) {
	...
	showAssetDialog_(assetInProgress) {
		...
		buttonsDiv.appendChild(
			this.makeButton_('Save', /* isFAB = */ false, () => {
		...
		buttonsDiv.appendChild(
			this.makeButton_('Cancel', /* isFAB = */ false, () => {
				this.dialog_.close();
			})
		);
	...
	remakeSavedList_() {
		...
		if (this.assets_.size == 0) {
			...
			makeMessage('body-1', 'Custom assets will remain even after reloading the page.');
		} else {
