...
class ShakaDemoSearch {
	...
	constructor(container) {
		...
		document.addEventListener('shaka-main-page-changed', () => {
			if (!this.resultsDiv_.childNodes.length && !container.classList.contains('hidden')) {
		...
		switch (type) {
			...
			case ShakaDemoSearch.TermType.FEATURE:
				...
				for (const term of others) {
					const index = this.desiredFeatures_.indexOf(/** @type {shakaAssets.Feature} */ (term));
					if (index != -1) {
	...
	removeDesiredTerm_(term, type) {
		...
		switch (type) {
			...
			case ShakaDemoSearch.TermType.SOURCE:
				...
				break;
			case ShakaDemoSearch.TermType.FEATURE:
				index = this.desiredFeatures_.indexOf(/** @type {shakaAssets.Feature} */ (term));
				if (index != -1) {
					this.desiredFeatures_.splice(index, 1);
				}
				break;
	...
	makeSelectInput_(searchContainer, name, choices, type) {
		...
		const input = new ShakaDemoSelectInput(searchContainer, name, onChange, valuesObject);
	...
	makeSearchDiv_(container) {
		...
		const coreContainer = new ShakaDemoInputContainer(
			container,
			/* headerText = */ null,
			ShakaDemoInputContainer.Style.FLEX,
			/* docLink = */ null
		);
		this.makeSelectInput_(coreContainer, 'Manifest', [Feature.DASH, Feature.HLS], FEATURE);
		this.makeSelectInput_(coreContainer, 'Container', [Feature.MP4, Feature.MP2TS, Feature.WEBM], FEATURE);
		this.makeSelectInput_(coreContainer, 'DRM', Object.values(shakaAssets.KeySystem), DRM);
		this.makeSelectInput_(
			coreContainer,
			'Source',
