...
class ShakaReceiverApp {
	...
	init() {
		...
		this.video_.addEventListener('play', () => this.onPlayStateChange_());
		this.video_.addEventListener('pause', () => this.onPlayStateChange_());
		this.video_.addEventListener('seeking', () => this.onPlayStateChange_());
		this.video_.addEventListener('emptied', () => this.onPlayStateChange_());

		this.receiver_ = new shaka.cast.CastReceiver(this.video_, this.player_, appData =>
	...
	checkIdle_() {
		...
		if (this.receiver_.isIdle()) {
			...
			if (this.video_.readyState != 0 && this.player_.isAudioOnly()) {
				this.video_.poster = 'https://shaka-player-demo.appspot.com/assets/audioOnly.gif';
			} else {
