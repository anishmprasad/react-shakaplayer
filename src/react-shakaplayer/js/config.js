...
class ShakaDemoConfig {
	...
	addDrmSection_() {
		...
		this.addSection_('DRM', docLink).addBoolInput_(
			'Delay License Request Until Played',
	...
	addManifestSection_() {
		...
		this.addSection_('Manifest', docLink)
			...
			.addBoolInput_('Auto-Correct DASH Drift', 'manifest.dash.autoCorrectDrift')
			.addBoolInput_('Xlink Should Fail Gracefully', 'manifest.dash.xlinkFailGracefully')
			.addNumberInput_(
				'Availability Window Override',
			...
			.addNumberInput_('Default Presentation Delay', 'manifest.dash.defaultPresentationDelay')
			.addBoolInput_('Ignore Min Buffer Time', 'manifest.dash.ignoreMinBufferTime');
	...
	addAbrSection_() {
		...
		this.addSection_('Adaptation', docLink)
			...
			.addNumberInput_('Default Bandwidth Estimate', 'abr.defaultBandwidthEstimate')
			.addNumberInput_('Bandwidth Downgrade Target', 'abr.bandwidthDowngradeTarget', /* canBeDecimal = */ true)
			.addNumberInput_('Bandwidth Upgrade Target', 'abr.bandwidthUpgradeTarget', /* canBeDecimal = */ true)
			.addNumberInput_('Switch Interval', 'abr.switchInterval', /* canBeDecimal = */ true);
	...
	addRetrictionsSection_(category, categoryName) {
		...
		const sectionName = (categoryName ? categoryName + ' ' : '') + 'Restrictions';
	...
	addRetrySection_(category, categoryName) {
		...
		this.addSection_(categoryName + ' Retry Parameters', docLink)
			...
			.addNumberInput_('Base Delay', prefix + 'baseDelay', /* canBeDecimal = */ true)
			.addNumberInput_('Backoff Factor', prefix + 'backoffFactor', /* canBeDecimal = */ true)
			.addNumberInput_('Fuzz Factor', prefix + 'fuzzFactor', /* canBeDecimal = */ true)
			.addNumberInput_('Timeout', prefix + 'timeout', /* canBeDecimal = */ true);
	}

	/** @private */
	addOfflineSection_() {
		const docLink = this.resolveExternLink_('.OfflineConfiguration');
		this.addSection_('Offline', docLink).addBoolInput_('Use Persistent Licenses', 'offline.usePersistentLicense');
	}
	...
	addStreamingSection_() {
		...
		this.addSection_('Streaming', docLink)
			.addNumberInput_('Maximum Small Gap Size', 'streaming.smallGapLimit', /* canBeDecimal = */ true)
			.addNumberInput_('Buffering Goal', 'streaming.bufferingGoal', /* canBeDecimal = */ true)
			.addNumberInput_('Duration Backoff', 'streaming.durationBackoff', /* canBeDecimal = */ true)
			.addNumberInput_('Rebuffering Goal', 'streaming.rebufferingGoal', /* canBeDecimal = */ true)
			.addNumberInput_('Buffer Behind', 'streaming.bufferBehind', /* canBeDecimal = */ true)
			.addNumberInput_('Safe Seek Offset', 'streaming.safeSeekOffset', /* canBeDecimal = */ true)
			.addNumberInput_('Stall Threshold', 'streaming.stallThreshold', /* canBeDecimal = */ true)
			.addNumberInput_('Safe Skip Distance', 'streaming.stallSkip', /* canBeDecimal = */ true);
		...
		if (!shakaDemoMain.getNativeControlsEnabled()) {
			...
			const tooltipMessage =
				'Text must always be streamed while native ' + 'controls are enabled, for captions to work.';
		...
		this.addBoolInput_('Jump Large Gaps', 'streaming.jumpLargeGaps')
			...
			.addBoolInput_('Start At Segment Boundary', 'streaming.startAtSegmentBoundary')
			.addBoolInput_('Ignore Text Stream Failures', 'streaming.ignoreTextStreamFailures')
	...
	addLanguageSection_() {
		...
		this.addNumberInput_('Preferred Audio Channel Count', 'preferredAudioChannelCount');
	}
	...
	addSection_(name, docLink) {
		const style = name ? ShakaDemoInputContainer.Style.ACCORDION : ShakaDemoInputContainer.Style.VERTICAL;
		this.sections_.push(new ShakaDemoInputContainer(this.container_, name, style, docLink));
	...
	addCustomBoolInput_(name, onChange, tooltipMessage) {
		...
		this.latestInput_ = new ShakaDemoBoolInput(this.getLatestSection_(), name, onChange);
		return this;
	}
	...
	addTextInput_(name, valueName, tooltipMessage) {
		...
		this.latestInput_.input().value = shakaDemoMain.getCurrentConfigValue(valueName);
		this.addedFields_.add(valueName);
		return this;
	}
	...
	addCustomTextInput_(name, onChange, tooltipMessage) {
		this.createRow_(name, tooltipMessage);
		this.latestInput_ = new ShakaDemoTextInput(this.getLatestSection_(), name, onChange);
		return this;
	}
	 ...
	 * @private
	 */
	addNumberInput_(name, valueName, canBeDecimal = false, canBeZero = true, canBeUnset = false, tooltipMessage) {
		const onChange = input => {
	...
	addDatalistInput_(name, values, onChange, tooltipMessage) {
		this.createRow_(name, tooltipMessage);
		this.latestInput_ = new ShakaDemoDatalistInput(this.getLatestSection_(), name, onChange, values);
		return this;
	}
	...
	addSelectInput_(name, values, onChange, tooltipMessage) {
		this.createRow_(name, tooltipMessage);
		this.latestInput_ = new ShakaDemoSelectInput(this.getLatestSection_(), name, onChange, values);
		return this;
	}
