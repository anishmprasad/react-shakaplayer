...
class Overlay {
	...
	getConfiguration() {
		...
		ConfigUtils.mergeConfigObjects(
			...
			this.config_,
			this.defaultConfig_(),
			/* overrides (only used for player config)*/ {},
			/* path */ ''
		);
		return ret;
	}
	...
	configure(config, value) {
		...
		if (this.config_.castReceiverAppId && !this.config_.overflowMenuButtons.includes('cast')) {
	...
	static scanPageForShakaElements_() {
		...
		// UI.
		const containers = document.querySelectorAll('[data-shaka-player-container]');
		...
		if (!videos.length && !containers.length) {
			...
			for (const container of containers) {
				...
				// one we saw.
				if (container['dataset'] && container['dataset']['shakaPlayerCastReceiverId']) {
	...
	static dispatchLoadedEvent_(eventName) {
		...
		const uiLoadedEvent = /** @type {!CustomEvent} */ (document.createEvent('CustomEvent'));
